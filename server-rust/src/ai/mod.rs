...
/// 生成菜单提示词
pub fn build_menu_prompt(meal_type: &str, servings: u8, preferences: &[String], restrictions: &[String]) -> String {
...
/// 调用 AI API（预留接口）
pub async fn call_ai_api(_prompt: String, _provider: &str, _api_key: &str) -> Result<Menu, AppError> {
    // 未来实现 AI API 调用
    // 目前返回错误，使用降级方案
